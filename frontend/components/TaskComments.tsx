...
export default function TaskComments({
  ...
  // Do not auto-load on mount; we'll load on expand or hover to avoid N requests
  useEffect(() => {
    // no-op
  }, [daoId, taskId]);
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex -space-x-2">
              {visible.map((c, idx) => (
                <Avatar
                  key={c.userId + idx}
                  className="h-6 w-6 ring-2 ring-white dark:ring-gray-900 first:ml-0 ml-[-0.5rem]"
                >
